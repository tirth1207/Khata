...
export default function HomeScreen() {
  ...
  return <View style={ui.screen}><ScrollView contentContainerStyle={ui.content}>
    ...
    <Card style={[s.hero,{backgroundColor:palette.primary,borderColor:palette.primary}]}><Text style={s.heroLabel}>TOTAL BALANCE</Text><Text style={[s.balance,{color:palette.primaryText}]}>{formatMoney(balance,currency as any)}</Text><View style={ui.row}><Metric label="Income" value={formatMoney(income,currency as any)} tone="income" ui={ui} palette={palette}/><Metric label="Expenses" value={formatMoney(expenses,currency as any)} tone="expense" ui={ui} palette={palette}/><Metric label="Saved" value={formatMoney(savings,currency as any)} ui={ui} palette={palette}/></View></Card>
    {accounts.length===0&&<Card style={{marginTop:14}}><Text style={[s.emptyTitle,{color:palette.text}]}>Start with your first account</Text><Text style={ui.subtitle}>Cash, bank, UPI, wallet — whatever represents money you control.</Text><Pressable onPress={()=>setSheet(true)}><Text style={[ui.action,{marginTop:12}]}>Add a transaction →</Text></Pressable></Card>}
    <SectionTitle title="This month"/>
    <Card><View style={ui.row}><View style={ui.grow}><Text style={ui.small}>SAVINGS RATE</Text><Text style={[s.big,{color:palette.text}]}>{income>0?Math.round(savings/income*100):0}%</Text></View><Text style={{color:savings>=0?palette.income:palette.expense,fontWeight:'700'}}>{savings>=0?'On track':'Over income'}</Text></View><View style={[ui.progressTrack,{marginTop:14}]}><View style={[ui.progressFill,{width:(income>0?Math.max(0,Math.min(100,savings/income*100)):0)+'%',backgroundColor:savings>=0?palette.income:palette.expense}]}/></View></Card>
...
function Metric({label,value,tone,ui,palette}:{label:string;value:string;tone?:'income'|'expense';ui:any;palette:any}){return <View style={{flex:1,marginTop:20}}><Text style={ui.small}>{label}</Text><Text style={[ui.amount,{marginTop:3,color:tone==='income'?palette.income:tone==='expense'?palette.expense:palette.primaryText}]}>{value}</Text></View>;}
const s=StyleSheet.create({hero:{marginTop:18},heroLabel:{fontSize:11,fontWeight:'800',letterSpacing:.7,color:'#A7A7AD'},balance:{fontSize:38,fontWeight:'700',letterSpacing:-1.4,marginTop:4},big:{fontSize:25,fontWeight:'700',marginTop:3},dot:{width:9,height:9,borderRadius:5,marginRight:10},icon:{width:36,height:36,borderRadius:12,alignItems:'center',justifyContent:'center',marginRight:11},name:{fontSize:15,fontWeight:'700'},emptyTitle:{fontSize:17,fontWeight:'700'}});
